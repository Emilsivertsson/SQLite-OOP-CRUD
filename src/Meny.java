public class Meny {

    public void mainMenu(){
        System.out.println("1. Add new person");
        System.out.println("2. Add new car");
        System.out.println("3. Add new workplace");
        System.out.println("4. Edit person");
        System.out.println("5. Edit car");
        System.out.println("6. Edit workplace");
        System.out.println("7. Delete person");
        System.out.println("8. Delete car");
        System.out.println("9. Delete workplace");
        System.out.println("10. Show all persons");
        System.out.println("11. Show all cars");
        System.out.println("12. Show all workplaces");
        System.out.println("13. Show all persons with a specific car");
        System.out.println("14. Show all persons with a specific workplace");
        System.out.println("15. Show all persons with a specific car and workplace");
        System.out.println("16. Show a person");
        System.out.println("17. Exit\"");

        System.out.println("Choose an option: ");
    }

    public void invalidInput(){
        System.out.println("Invalid input, please try again");
    }


}
